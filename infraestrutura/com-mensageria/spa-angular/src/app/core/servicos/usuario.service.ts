...
export class UsuarioService {
  ...
  findAll(sort: string, order: string, page: number, pageSize: number)
    ...
    const requestUrl = `${URL.baseUrlUsuarios}${URL.usuariosAll_Get}`;
  ...
  saveOrCreate(usuario: UsuarioEntityTo): Observable<UsuarioEntityTo> {
    ...
    const requestUrl = `${URL.baseUrlUsuarios}${URL.usuariosSaveOrCreate_Post}`;
    return this.http.post<UsuarioEntityTo>(requestUrl, usuario);
  }

  remove(id: number): Observable<void> {

    const requestUrl = `${URL.baseUrlUsuarios}${URL.usuariosRemove_Delete}`
