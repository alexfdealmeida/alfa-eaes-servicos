...
export class TarefaService {
  ...
  findAll(sort: string, order: string, page: number, pageSize: number)
    ...
    const requestUrl = `${URL.baseUrlTarefas}${URL.tarefasAll_Get}`;
  ...
  saveOrCreate(tarefa: TarefaEntityTo): Observable<TarefaEntityTo> {
    ...
    const requestUrl = `${URL.baseUrlTarefas}${URL.tarefasSaveOrCreate_Post}`;
    return this.http.post<TarefaEntityTo>(requestUrl, tarefa);
  }

  remove(id: number): Observable<void> {

    const requestUrl = `${URL.baseUrlTarefas}${URL.tarefasRemove_Delete}`
